...
import { Search, Car, Euro, MapPin, Sliders, X } from "lucide-react";
    ...
    <div className="relative mx-auto w-full max-w-[85rem] overflow-hidden rounded-2xl bg-gradient-to-br from-red-500 via-blue-600 to-red-700 shadow-xl">
      <div className="flex min-h-[400px] flex-col lg:flex-row rounded-2xl border border-white/20 bg-white/10 shadow-lg backdrop-filter backdrop-blur-xl">
        ...
        <div className="flex w-full flex-col justify-between p-4 sm:p-6 lg:w-1/3 bg-white/10 backdrop-blur-xl rounded-2xl lg:rounded-r-none order-2 lg:order-1">
          <div>
            <h2 className="mb-4 sm:mb-6 text-center text-lg sm:text-xl font-semibold text-white">Find Your Perfect Car</h2>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="flex space-x-2 sm:space-x-3">
                <div className="relative flex-1">
                  <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                    <Car className="h-4 sm:h-5 w-4 sm:w-5 text-blue-200" />
                  </div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="outline"
                        className="h-10 sm:h-12 w-full justify-start rounded-xl sm:rounded-2xl border-none bg-white/10 text-left text-sm text-white ring-1 ring-inset ring-white/20 pl-10"
                      >
                        {selectedMake || "Make"}
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="w-full rounded-xl sm:rounded-2xl">
                      {carBrands.map((brand) => (
                        <DropdownMenuItem
                          key={brand}
                          onSelect={() => setSelectedMake(selectedMake === brand ? null : brand)}
                          className={`cursor-pointer text-sm ${
                            selectedMake === brand ? "bg-blue-100 font-semibold text-blue-800" : ""
                          }`}
                        >
                          {brand}
                        </DropdownMenuItem>
                    ...
                    <DropdownMenuContent className="w-full rounded-xl sm:rounded-2xl">
                      {selectedMake &&
                        carModels[selectedMake]?.map((model) => (
                          <DropdownMenuItem
                            key={model}
                            onSelect={() => setSelectedModel(selectedModel === model ? null : model)}
                            className={`cursor-pointer text-sm ${
                              selectedModel === model ? "bg-blue-100 font-semibold text-blue-800" : ""
                            }`}
                          >
                            {model}
                          </DropdownMenuItem>
              ...
              <div className="flex space-x-2 sm:space-x-3">
                <div className="relative flex-1">
                  <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                    <Euro className="h-4 sm:h-5 w-4 sm:w-5 text-blue-200" />
                  </div>
                  <Input
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="w-full rounded-xl sm:rounded-2xl">
                    {bodyTypes.map((type) => (
                      <DropdownMenuItem
                        key={type}
                        onSelect={() => setSelectedBodyType(selectedBodyType === type ? null : type)}
                        className={`cursor-pointer text-sm ${
                          selectedBodyType === type ? "bg-blue-100 font-semibold text-blue-800" : ""
                        }`}
                      >
                        {type}
                      </DropdownMenuItem>
