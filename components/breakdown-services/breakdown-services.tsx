...
interface BreakdownServicesProps {
  className?: string;
  showHeader?: boolean;
  showFooter?: boolean;
...
export function BreakdownServices({
  className = "",
  showHeader = true,
  showFooter = true,
  customServices
}: BreakdownServicesProps) {
  ...
  return (
    <div className={`bg-white min-h-screen ${className}`}>
        ...
        <div className="bg-gray-100 py-6">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between">
              <h1 className="text-3xl font-bold text-blue-900">Breakdown Services</h1>
              <div className="flex items-center space-x-2">
                <div className="text-sm">
                  <span className="text-gray-600">Home</span>
                  <ChevronRight className="inline h-4 w-4 text-gray-400" />
                  <span className="text-blue-600">Breakdown Services</span>
                </div>
              </div>
            </div>
          </div>
            ...
            {Object.entries(services).map(([key, service]) => (
              ...
              >
                <div className="p-6">
                  ...
                  <button className={`w-full py-2 px-4 rounded-md ${selectedService === key ? 'bg-blue-900 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>
      ...
      {showFooter && (
        <div className="bg-gray-100 py-6">
          <div className="container mx-auto px-4 text-center">
            <p className="text-gray-600">© {new Date().getFullYear()} GB Trader. All rights reserved.</p>
          </div>
        </div>
      )}
    </div>
