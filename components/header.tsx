...
export function Header() {
  ...
  const bottomNavItems = [
    ...
    { href: "/car-reviews", label: "Car Reviews" },
    { href: "/dealers", label: "Dealers" },
    { href: "/buy-a-car", label: "Buy a Car" },
    { href: "/search-vehicle", label: "Search Vehicle" },
  ]
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex flex-col items-center flex-grow mx-8">
            {/* Top navigation with gray background */}
                  ...
                  <nav className="flex flex-col space-y-3">
                    {bottomNavItems.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="text-sm font-medium py-2 border-b border-gray-100 hover:text-primary"
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {item.label}
                      </Link>
